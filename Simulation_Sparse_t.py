#!/usr/bin/env python3
"""
Sparse SYK Trotter-error GUI demo (Bernoulli mask) — sweep over time t (n fixed)
...
- Fixed n; sweep t in [0, 10] with T_POINTS samples (default 1000)
- Two progress bars per worker: Overall (3 ticks per repeat) and Step-2 (active multiplies only)
- NEW: record active_count = len(active_idx) in the database (same schema as n-sweep)
"""
...
PROC_COUNT = max(2, (os.cpu_count() - 4 or 2))   # leave ~4 cores idle if possible
N_WORKERS  = PROC_COUNT - 1                       # 1 overseer + N_WORKERS workers

# FIXED n (even). Dimension is 2^(n//2).
N_FIXED = 10

# Sweep t over [0, 10], default 1000 points
...
# Database file (includes n fixed)
os.makedirs("DATABASE", exist_ok=True)
DATABASE = f"DATABASE/DATA_SPARSE_t_sweep(n={N_FIXED},k={k},l={l},r={r},J={round(J_E,3)}).txt"
...
def S_with_progress(
    ...
    Step-2 bar advances once per local exponential multiply executed.
...
def worker_loop(
    ...
    For each ("iter", iter_id, payload) with fixed n and varying t:
      Repeat rep_id = 0..REPEATS_PER_WORKER-1:
        1) sample Bernoulli B and Gaussian J (Overall +1)
        2) build S_l using ONLY active terms (Step-2 progress only)
        3) exact U = expm(i t H) with H = sum_{i in active} J_i K_i (Overall +1)
        4) Schatten p-norm of difference (Overall +1)
        5) send ("done_rep", wid, iter_id, rep_id, msg, rand, norm, active_count)  # NEW
    """
    outq.put(("hello", wid))
    while True:
        cmd = inbox.get()
        ...
        # Bernoulli prob p_B = n / G  (G = comb(n, k_))
        p_B = n / float(G)
        ...
        for rep_id in range(REPEATS_PER_WORKER):
            ...
            B = (rng.random(G) < p_B)
            active_idx = np.nonzero(B)[0].astype(np.int64)
            active_count = int(active_idx.size)                   # NEW: track active_count
            ...
            # 2) Build S_l on active terms (or identity if none)
            if active_count == 0:
            ...
            # 5) Report (include active_count)  # NEW
...
class OverseerGUI:
    def __init__(self, n_workers: int) -> None:
        ...
        # GUI
        self.root = tk.Tk()
        self.root.title("Overseer – Trotter Error (Bernoulli, t-sweep, Repeats + Two-Bar Workers)")
        self.root.geometry("1400x950")  # wide + tall
        ...
        # store by (iter_idx, wid, rep) -> (msg, rand, norm, active_count)
        ...
        # Kick off first t after GUI shows
    ...
    def _build_gui(self) -> None:
        ...
        self.iter_bar = ttk.Progressbar(it, orient="horizontal", mode="determinate",
                                        length=1000, maximum=len(T_VALUES), value=0)
    ...
    def _ensure_db_header(self) -> None:
        if not os.path.isfile(DATABASE):
            with open(DATABASE, "w") as f:
                # SAME schema as the n-sweep version (includes active_count)
                f.write("n,k,t,l,r,p,rep,norm,active_count,timestamp,worker\n")

    def _append_db_line(self, n: int, t_cur: float, rep: int, norm: float, active_count: int, wid: int) -> None:
        now = datetime.datetime.now().isoformat(timespec="seconds")
        with open(DATABASE, "a") as f:
            f.write(f"{n},{k},{t_cur},{l},{r},{p},{rep},{norm},{active_count},{now},{wid}\n")

    # ----- Iteration driver (over t) -----
    ...
    def _handle_message(self, msg: Tuple) -> None:
        ...
        if kind == "rep_start":
            ...
            # NEW tuple includes active_count
            _, wid, iter_id, rep_id, echoed_msg, randval, normval, active_count = msg
            if iter_id != self.cur_iter_index: return

            # Store results keyed by iteration index (t index)
            self.results[(iter_id, wid, rep_id)] = (str(echoed_msg), float(randval), float(normval), int(active_count))

            # Append to DB with actual t for this iteration, including active_count  # NEW
            t_cur = float(T_VALUES[self.cur_iter_index])
            self._append_db_line(N_FIXED, t_cur, rep_id, float(normval), int(active_count), wid)
            ...
            # When all workers finished all reps for this t, advance
        ...
        for wid in range(self.n_workers):
            for rep_id in range(REPEATS_PER_WORKER):
                msg, rnum, norm, act = self.results.get(
                    (iter_idx, wid, rep_id), ("<missing>", float("nan"), float("nan"), -1)
                )
                lines.append(
